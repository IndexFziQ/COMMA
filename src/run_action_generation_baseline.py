...
# GPT2DoubleHeadsMaskModel
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          BertConfig, BertForMaskedLM, BertTokenizer,
                          GPT2Config, GPT2DoubleHeadsMaskModel, GPT2Tokenizer,
...
MODEL_CLASSES = {
    'gpt2': (GPT2Config, GPT2DoubleHeadsMaskModel, GPT2Tokenizer),
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', max_len=128,
        ...
        cached_features_file = os.path.join(directory,
                                            args.model_name_or_path + '_bs_cached_lm_' + str(max_len) + '_' + filename)
        cached_features_list = os.path.join(directory,
                                            args.model_name_or_path + '_bs_cached_list_' + str(max_len) + '_' + filename)
        cached_features_label = os.path.join(directory,
                                            args.model_name_or_path + '_bs_cached_label_' + str(max_len) + '_' + filename)
        ...
        if os.path.exists (cached_features_label) and not args.overwrite_cache:
            logger.info ("Loading features from cached file %s", cached_features_label)
            with open (cached_features_label, 'rb') as handle3:
                self.label_ids = pickle.load(handle3)
        else:
            ...
            with open (file_path, 'r', encoding='utf-8') as csv_file:
                ...
                for row in reader:
                    record = row
                    # all key words:
                    # storyid_linenum,char,context,sentence,label_emotion,label_motive,next_motive,next_event

                    # storyid_linenum = record['storyid_linenum']
                    human = record['char']
                    context = record['history']
                    event = record['behavior']
                    # desc = json.loads (record['srl_annotation'])
                    ...
                    # concepts = tokenization(event)
                    # concepts = lower_case(concepts)

                    # if len(concepts)>0:
                    text_ = context + " <Beh>"
                    # text_ = context + " <Mot> The motive of %s is %s . </Mot>" % (human, appendix) +  \
                    #         "The emotion of %s could be %s ." % (human, emotions_set)  + "<Beh>"
                    text_pair = event + "</Beh>"
                    # text_ = "<mot> The motive of %s is %s . </mot> " % (human, appendix) \
                    #         + '<cpt> ' + ', '.join (random.choice(concepts)) \
                    #         + '</cpt> ' + "<beh>"
                    # text_ = "<emo> The emotion of %s could be %s . </emo> " % (human, emotions_set) \
                    # text_ = '<cpt> ' + ', '.join (random.choice(concepts)) \
                    #         + '</cpt> ' + "<beh>"
                    # text_pair = event + "</beh>"
                    ...
                    label_list= [str(i) for i in range(1, 9)]
                    label_map = {label: i for i, label in enumerate (label_list)}
                    label_id = label_map[label_emotion]  if label_emotion is not None else -1
            ...
            logger.info ("Saving features into cached file %s", cached_features_label)
            with open (cached_features_label, 'wb') as handle3:
                pickle.dump(self.label_ids, handle3, protocol=pickle.HIGHEST_PROTOCOL)

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, item):
        return torch.tensor(self.examples[item]), torch.tensor([self.input_lens[item]]), torch.tensor([self.label_ids[item]])
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch_id in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            emotion_labels = batch[2]
            inputs, labels = mask_tokens(batch, tokenizer, args) if args.mlm else (batch[0], batch[0])
            inputs = inputs.to(args.device)
            labels = labels.to(args.device)
            emotion_labels = emotion_labels.to(args.device)
            model.train()
            outputs = model(inputs, lm_labels=labels, mc_labels=emotion_labels) if args.mlm else model(inputs, lm_labels=labels, mc_labels=emotion_labels, loss_mask=mask)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            outputs = model (inputs, lm_labels=labels,
                             mc_labels=emotion_labels) if args.mlm else model (inputs, lm_labels=labels,
                                                                               mc_labels=emotion_labels, loss_mask=mask)

            all_loss = outputs[0]

            eval_loss += all_loss.mean().item()
        nb_eval_steps += 1
