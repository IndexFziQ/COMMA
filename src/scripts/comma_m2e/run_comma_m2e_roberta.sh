...
MODEL_PATH=/your_model_path/pytorch-roberta-large
TK_PATH=/your_model_path/pytorch-roberta-large

OUTPUT_BASE=/your_checkpoint_path/checkpoints_comma_roberta_m2e
...
for LR_RATE in ${lr_iter[@]}; do
    for EPOCH_N in ${epoch_iter[@]}; do
        ...
        python -u /your_file_path/COMMA/src/run_bert_mber_m2e.py \
        --task_name ${TASK_NAME} \
        --data_dir ${DATA_DIR} \
        --num_choices 8 \
        --model_type roberta \
