...
class Trainer:
    ...
    def prediction_loop(
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            ...
            metrics = {}
        metrics[0].update(metrics[1])
        if eval_loss is not None:
            metrics[0][f"{metric_key_prefix}_loss"] = eval_loss.mean().item()

        # Prefix all keys with metric_key_prefix + '_'
        for key in list(metrics[0].keys()):
            if not key.startswith(f"{metric_key_prefix}_"):
                metrics[0][f"{metric_key_prefix}_{key}"] = metrics[0].pop(key)

        return PredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics[0])
